...
pusher_client = pusher.Pusher(
  app_id='804778',
  key='7cc23d3b24b1b23fe7bd',
  secret='30e7b3b5a38d01052480',
  cluster='us2',
  ssl=True
)
...
def authenticate():
    ...
    db_session = db.getSession(engine)
    try:
        user = db_session.query(entities.User).filter(entities.User.username == username).filter(
            entities.User.password == password).first()
